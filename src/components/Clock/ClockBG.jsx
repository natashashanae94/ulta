...
const ClockBG = ({bg}) => { 
    ...
    return(
        <>
            ...
            {bg.map((hour, index) => {
                const image = `${process.env.PUBLIC_URL}${hour.bg}`;
                return (
                <>
                    ...
                    <article className="project w-xs-100 h-xs-100 clearfix">
                            ...
                            <div className="project__panel project__thumbnail w-xs-50 h-xs-100 overflow-hidden">
                                <a href={url}>
                                    <div className="project__thumbnail__image w-xs-100 h-xs-100 background-cover lazypreload lazyloaded" style={{ backgroundImage: `url("${hour.bg}")`}}></div>
                                </a>
                            </div>
                        </article>
                    </React.Fragment>
